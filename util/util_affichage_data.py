...
def taux_incendie_par_quantiles(
    data: pd.DataFrame,
    features: list[str] | None = None,
    q: int = 5,
) -> None:
    """
    Montre l'effet de variables météo sur le taux d'incendie via des quantiles.
    Par défaut, analyse quelques features clés.
    """
    if features is None:
        features = [
            'PRECIPITATION', 'LAGGED_PRECIPITATION', 'AVG_WIND_SPEED', 'MAX_TEMP', 'TEMP_RANGE'
        ]

    df = assurer_colonnes_temporelles(data, besoin=("DATE",))
    n = len([f for f in features if f in df.columns])
    if n == 0:
        print("Aucune feature disponible pour les quantiles.")
        return
    ...
    for f in features:
        ...
        # qcut sur le rang pour gérer ex-aequo
        qbin = pd.qcut(df[f].rank(method='first'), q, labels=False)
        rate = df.groupby(qbin)['FIRE_START_DAY'].mean()
        ax.bar(range(1, len(rate) + 1), rate.values, color='steelblue')
        ax.set_title(f"Taux incendie vs quantiles de {f}")
        ax.set_xlabel("Quantile (faible → fort)")
    ...
    plt.suptitle("Effet des variables météo par quantiles sur le taux d'incendie", y=1.02)
    plt.tight_layout()
    plt.show()


def heatmap_taux_incendie_wind_temp(
    data: pd.DataFrame,
    bins_wind: int = 10,
    bins_temp: int = 10,
    farenite_en_degre: bool = False,
) -> None:
    """
    Carte 2D du taux d'incendie en fonction de la vitesse du vent (x)
    et de la température max (y), binnings réguliers.
    """
    df = assurer_colonnes_temporelles(data, besoin=("DATE",))
    if not {'AVG_WIND_SPEED', 'MAX_TEMP'}.issubset(df.columns):
        print("Colonnes requises manquantes: AVG_WIND_SPEED et MAX_TEMP")
        return

    dfx = df.copy()
    if farenite_en_degre:
        dfx['MAX_TEMP'] = (dfx['MAX_TEMP'] - 32) * 5 / 9

    wind_bins = pd.cut(dfx['AVG_WIND_SPEED'], bins=bins_wind)
    temp_bins = pd.cut(dfx['MAX_TEMP'], bins=bins_temp)
    tbl = dfx.groupby([temp_bins, wind_bins])['FIRE_START_DAY'].mean().unstack()

    plt.figure(figsize=(10, 6))
    sns.heatmap(tbl, cmap='YlOrRd', vmin=0, vmax=tbl.max().max(), cbar_kws={'label': 'Taux moyen'})
    plt.title("Taux d'incendie selon vent et température")
    plt.xlabel("Vitesse du vent (bins)")
    plt.ylabel("Température max (bins)")
    plt.tight_layout()
    plt.show()
